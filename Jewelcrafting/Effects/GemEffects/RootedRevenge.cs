	...
	private static IEnumerator IncreaseDamage(Player player)
	{
		while (true)
		{
			...
			if (config.Duration > 0 && !player.IsDead() && !Utils.SkipBossPower())
			{
				...
				foreach (Character character in Character.s_characters.Where(c => c != player && Vector3.Distance(c.transform.position, player.transform.position) <= config.Range && (BaseAI.IsEnemy(c, player) || (c is Player enemy && enemy.IsPVPEnabled()))).ToList())
				{
