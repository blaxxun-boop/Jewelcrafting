	...
	private static class IncreaseSkillLevel
	{
		private static void Postfix(Skills __instance, Skills.SkillType skillType, ref float __result)
		{
			if (skillType == Skills.SkillType.Bows)
			{
				...
				__result = Mathf.Max(level, Utils.GetNearbyGroupMembers(__instance.m_player, 20).Select(p => Mathf.Max(level + (p.m_nview.GetZDO().GetInt("Jewelcrafting Bows Skill") - level) * p.GetEffect(Effect.Archerymentor) / 100f)).DefaultIfEmpty(0).Max());
			}
		}
	}
