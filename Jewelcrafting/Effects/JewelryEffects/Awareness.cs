...
public static class Awareness
{
	...
	private static class DisplayIcon
	{
		private static void Postfix()
		{
			...
			if (Character.s_characters.Any(c => Vector3.Distance(playerPos, c.transform.position) < Jewelcrafting.awarenessRange.Value && c.GetComponent<MonsterAI>()?.IsAlerted() == true))
			{
				attackedIcon.gameObject.SetActive(true);
				heardIcon.gameObject.SetActive(false);
			}
			else if (Character.s_characters.Any(c => Vector3.Distance(playerPos, c.transform.position) < Jewelcrafting.awarenessRange.Value && c.GetComponent<MonsterAI>()?.HaveTarget() == true))
			{
				attackedIcon.gameObject.SetActive(false);
