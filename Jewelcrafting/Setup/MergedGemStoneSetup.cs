...
public static class MergedGemStoneSetup
{
	...
	public static void initializeMergedGemStones(AssetBundle assets)
	{
		...
		foreach (KeyValuePair<GemType, MaterialColor> first in GemStoneSetup.Colors)
		{
			mergedGems[first.Key] = new Dictionary<GemType, GameObject[]>();
			foreach (KeyValuePair<GemType, MaterialColor> second in GemStoneSetup.Colors)
			{
	...
	public static void CreateMergedGemStone(KeyValuePair<GemType, MaterialColor> first, KeyValuePair<GemType, MaterialColor> second)
	{
		...
		foreach (MergedGemAsset asset in mergedGemAssets)
		{
			...
			void SetColor(string location, KeyValuePair<GemType, MaterialColor> kv)
			{
				if (kv.Value.Material is { } material)
				{
					prefab.transform.Find($"attach/{location}").GetComponent<MeshRenderer>().material = material;
				}
				else
				{
					prefab.transform.Find($"attach/{location}").GetComponent<MeshRenderer>().material.color = kv.Value.Color;
					prefab.transform.Find($"attach/{location}").GetComponent<MeshRenderer>().material.SetColor(EmissionColor, kv.Value.Color);
				}
			}
